...
if (isset($_POST["r_post_id"]) && isset($_POST["username"])) {
	$post_id = $_POST["r_post_id"];
	$user_name = $_POST["username"];
	//deletePost($post_id);
	echo "<span class='d-flex justify-content-center text-center'> Post {$post_id}, from {$user_name} has been <b> deleted! </b> </span>";
}

$sql = "SELECT *, id AS r_post_id,
(
...
<body>
	...
	<div class="wrapper d-flex">
		...
		<!-- Page Content  -->
		<main>
			<div id="content" class="container d-flex flex-column justify-content-center align-items-center p-4 p-md-5 pt-5 ms-5">
								...
								<div class="card-body me-4 d-flex flex-row">
									...
									<div class="px-2 align-middle text-center justify-content-center align-items-center">
										<div class="d-flex flex-column me-3 mt-1">
											...
											<span class="mx-1" id="vote-count"><?= $row['total_likes'] ?></span>
									...
									<div class="d-flex flex-column w-100">
										...
										<div class="feedback-container d-flex flex-row justify-content-center my-2">
											<button class="px-2 py-2" style="max-width: 10rem"><i class="fa-solid fa-comment" style="color: grey;"></i>
												<span class="mx-auto my-auto" style="font-weight: bold; color: #6B6B6B"><?= $row['total_comment'] ?> comments</span></button>
											<!-- button here need fix  -->
											<form action="admin_posts.php" method='post'>
												<input type="text" name='r_post_id' value="<?= $row["r_post_id"] ?>" hidden>
												<input type="text" name='username' value="<?= $row["username"] ?>" hidden>
												<button type="submit" class="adminBtn btn btn-danger mx-auto" id="deletePost-<?= $row["post"] ?>-<?= $row["username"] ?>">Delete Post</button>
											</form>
											<a onMouseOver="this.style.textDecoration='underline'" onMouseOut="this.style.textDecoration='none'" class="post-detail-link ms-auto align-self-end" style="text-decoration: none; color: #6B6B6B; font-size: 13px; " href="post_detail.php">???</a>
										</div>
									</div>
								</div>
							</div>
